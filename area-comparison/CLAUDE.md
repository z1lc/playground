...
- **Extra Atlanta neighborhoods (limited data)**: Sweet Auburn, Cabbagetown, Reynoldstown, Inman Park, Old Fourth Ward, and Piedmont Heights are included as opt-in neighborhood columns (hidden by default; enable via the ⚙️ settings menu). They share the Statistical-Atlas-sourced age, race, national-origin, marital-status, household-income, educational-attainment, and employment fields, plus the limited walkability, housing, crime, tree-canopy, restaurant, and noise fields documented below. Metrics without neighborhood-level source data render blank (`—`). Piedmont Heights data was added in July 2026.
- **Atlanta neighborhood population override**: The seven displayed Atlanta columns use boundary-matched 2020–2024 ACS population estimates generated by `fetch_population.py`, not Statistical Atlas population. The script assigns 2020 Census blocks to eight official City neighborhood polygons using each block's internal point and allocates ACS table B01003 block-group estimates in proportion to 2020 block population. Virginia Highland and Morningside/Lenox Park cross the Fulton–DeKalb county line and are summed for the combined column. Density uses exact official polygon acreage. Runtime outputs: Virginia Highland + Morningside 19,339 / 5,846.66 per sq mi · Sweet Auburn 3,291 / 10,442.44 · Cabbagetown 1,851 / 10,561.11 · Reynoldstown 4,558 / 7,377.46 · Inman Park 5,412 / 9,027.52 · Old Fourth Ward 14,153 / 11,566.75 · Piedmont Heights 2,436 / 5,020.25. `CENSUS_API_KEY` is read from the environment (local key in gitignored `KEYS_census`). `data.json` intentionally retains the raw Statistical Atlas population and density alongside its other raw demographic extracts; the runtime population object is the authoritative override.
...
- **Tree Canopy Coverage (neighborhoods)**: Census-block-group-level tree canopy percentage from American Forests / USDA Community Forestry ArcGIS feature service (Census_Block_Groups_Land_Cover_and_Tree_Canopy_Analysis), using the `To_TC_Pct` field (Total Tree Canopy Over Impervious and Pervious Area Percent). Aggregated to neighborhoods via area-weighted averaging of all block groups within overlapping census tracts. Census tracts identified via Census Bureau geocoder point sampling within each neighborhood boundary.
  ...
  - Extra Atlanta neighborhoods (via `fetch_tree_canopy.py`): area-weighted (by `Land_Ac`) mean of `To_TC_Pct` across the census **block groups** intersecting each neighborhood's bounding box — finer than the tract-based method above, appropriate for these small neighborhoods. Inman Park 44.5%, Reynoldstown 44.7%, Cabbagetown 30.5%, Sweet Auburn 16.5%, Old Fourth Ward 31.5%, Piedmont Heights 42.9%. Old Fourth Ward uses the tight extent of City of Atlanta Neighborhood layer OBJECTID 201. Piedmont Heights uses the official City layer OBJECTID 35 extent `(-84.3776781, 33.7951677, -84.3658086, 33.8134788)`.
...
- **Home Price Index (neighborhoods)**: Zillow ZHVI (all homes, middle tier). Downloaded CSV: Neighborhood_zhvi_uc_sfrcondo_tier_0.33_0.67_sm_sa_month.csv. RegionIDs: NP 417522, AV 417521, GV 195133, VH 275890, ML 274592. Extra Atlanta neighborhoods: Inman Park 272817, Reynoldstown 269422, Cabbagetown 127516, Old Fourth Ward 269397, Piedmont Heights 403427 (the same-named Duluth, Minnesota neighborhood is RegionID 763862 and must not be used; Sweet Auburn 276600 is omitted because its ZHVI starts in 2014, with no 2000 baseline or bedroom-specific data). Normalized to 100 at 2000, annual Jan values 2000-2025. NP+AV and VH+ML averaged. The extra Atlanta neighborhoods are single-region (not paired) and are regenerated by `fetch_zillow.py` (keyed by RegionID; caches CSVs in gitignored `raw_zillow/`). Piedmont Heights has 26 index points, from 100.0 to 240.5, for a 3.6% CAGR.
...
- **ATL (Virginia Highland + Morningside, and 6 extra neighborhoods)**:
